...
Class NotGosm extends CI_Controller{
	...
	$result = $this->Arts_Model->getOrgID( $this->input->post('OrgName'));
		   if($result) {
			 foreach($result as $row) {
			    ...
			    $checkSub= $this->Arts_Model->checkNumNotGosm($this->input->post('OrgName'), $term);
		   		if($checkSub)
		   		{
		   			foreach($checkSub as $Sub)
		   			{
		   				if($Sub->count < 10){
				   		$checkNum= $this->Arts_Model->checkNotGosm($this->input->post('OrgName'), $schoolyear);
				   		if($checkNum)
				   		{
				   			foreach($checkNum as $res)
				   			{
							   		if($res->count < 10)
							   		{
										    ...
										    $check = $this->Arts_Model->checkTitle($this->input->post('ActTitle'),$this->input->post('OrgName'), $this->input->post('TermID'));
												if(!$check) {
													$data = array(
											         ...
											         'gendetails_Related' => $this->input->post('Related'));
													...
													$result1 = $this->Arts_Model->getProjID($this->input->post('ActTitle'), $this->input->post('OrgName'), $this->input->post('TermID'));
												  	...
												  	echo '<label style="font-size :20px">Submission</label>
													<button  align = "right"type="button" class="btn btn-default" data-dismiss="modal" style="float: right;">Close</button><br>
													<hr><div class="success" style="text-align: center;">
								  		  			Submission Failed<br>
									...
									else{ //else checkNotGosm
											echo '<label style="font-size :20px">Submission</label>
											<button  align = "right"type="button" class="btn btn-default" data-dismiss="modal" style="float: right;">Close</button><br>
											<hr>
											<div class="success" style="text-align: center;">
								  		  	Submission Failed<br>
								  		  	You have exceeded the allowable number of Not in GOSM submission.
								  		  	</div>
								  		 	<div class="modal-footer">
								  		 	<a href="NotGosm" class="btn btn-default">Submit Again</a>
								  		 	</div>';
						...
						<hr>
			  			<div class="success" style="text-align: center;">
			  		 	Submission Failed<br>
			  		  	Your Not In GOSM submission has exceeded 10 submissions already<br>
			  		  	Please wait for the Approval first.<br>
			  		  	</div>
			  		  	<div class="modal-footer">
			  		  	<a href="NotGosm" class="btn btn-default">Submit Again</a>
			  		  	</div>';
