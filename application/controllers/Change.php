	...
	if($this->form_validation->run() == TRUE) 
	{
		if(strtotime(date('F d, Y')) <= strtotime($date)){
			   ...
			   if($result) {
				    ...
				    $check = $this->Arts_Model->checkChange($this->input->post('ActTitle'),$this->input->post('OrgName'), $this->input->post('TermID'));
					if($check) {
						$data = array(
				 	...
				 	else // else checkTitle
					   {
					  	echo '<label style="font-size :20px">Submission</label>
								...
								<hr>
						  		<div class="success" style="text-align: center;">
			  		  			Submission Failed<br>
			  		  			You cannot submit this activity yet <br>"'
			  		  			. $this->input->post('ActTitle')	. '" has not been approved in your previous submission
			  		  			</div>
			  		 			<div class="modal-footer">
			  		 			<a href="NotGosm" class="btn btn-default">Submit Again</a>
			  		 			</div>';
